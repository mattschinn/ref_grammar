...
➡ **Milestone A COMPLETE. B1a (data port) COMPLETE (2026-06-28).** 16 grammar examples centralized (E002–E017), two cross-doc duplicates collapsed, inline copies retained. **Design refined 2026-06-28** (greedy-blockable rendering + SSOT-deferring preview) — this re-scopes B1b off the D4 dependency and onto **G0a** (greedy renderer) + **G0b** (preview-sync). **G0a + G0b + B1b COMPLETE (2026-06-28).** The greedy-blockable renderer, auto-regenerated SSOT-deferring previews (single renderer; PDF builds just strip), and full tokenization of all 20 grammar example sites (corpus now E001–E019) are done and verified. The centralized-examples system is end-to-end live: examples.md is the sole source, the docs carry tokens+auto-synced previews, and both pipelines render from one place. **Next live tasks: B2** (import `/drafts` translation exercises per the draft-import bar) and **C1** (codify examples pacing) — both author-facing. Deferred polish: **G1** (HTML interactive hover-highlight), **G2** (PDF monospace columns). PDF still wants one real pandoc/tectonic render to confirm the strip path end-to-end.
...
- [x] **B1b. Tokenize grammar examples (prose).** ✓ 2026-06-28. Replaced all 20 inline blocks with `<!-- example: EXXX -->` tokens (+ auto-synced previews): `verbal-system.md` E002–E018 (17), `ideophones.md` E003/E010 (2), `language_reference.md` E019 (1). The first-pass survey had missed two (the conditional E018 in `verbal-system.md` §5.5, the existential E019 in `language_reference.md` §4 — survey regex required a capitalized first word and skipped langref); both added to the corpus. Cross-doc duplicates render greedily in `ideophones.md` (χ shown) and `| -gesture` in `verbal-system.md` (χ scoped to its defining doc) — first real use of blockable layers. E008/E017 inside-block caveats kept as following notes. Versioning notes added to all four docs (`examples.md` v0.4, `verbal-system.md` v2.13, `ideophones.md` v1.2, `language_reference.md` v3.11). Build clean (19 examples, 6 pages); zero glossed blocks remain. **Milestone B corpus-population now complete except B2 (draft import).**
- [~] **B2. Import draft translations.** *TX#2 complete; other sources pending.* Per the locked draft-import bar. **Translation Exercise #2 fully imported 2026-06-28:** E020–E056 (pilot E020–E022 + bulk E023–E056), all **[Provisional — review pending]**, corpus-only (transcluded nowhere), source pointers + stub-checks + conflict flags, **reconciled-to-canon** (author-confirmed) with Etymological deferred (draft brace-gloss captured in Notes). ~12 of TX#2's sentences were already E001–E019. **Deferred (not imported):** the 3 negation-bearing hearsay sentences (TX#2 §B.5) — clash with unsettled negation (`drafts/negative-*`). **Still to survey:** `translation_exercise_1/`, `poetics.md`, `verb-concept-grids`, `negatives_*` (the last gated on the negation system). **Awaiting author review** of E020–E056 (formats, reconciliations, uncertain frame forms *retmseto*/*ritmseto*/*rennausseto*).
...
- 2026-06-28 — **G0b + B1b done.** G0b: extracted the render core to `examples-render.mjs`; `sync-previews.mjs` auto-regenerates a `<!-- preview -->`…`<!-- /preview -->` block after each token in the canonical docs (inline or block per render shape), wired into `build-content.mjs` (sync→strip) and both PDF builds (`node sync` → PowerShell `Strip-Examples`, no layer logic PS-side). `parseDictionary` drops the token+preview from short-defs. Verified idempotent + code-safe; PS scripts parse-check + unit-tested. B1b: tokenized all 20 grammar sites (E002–E019; found 2 missed examples E018/E019); `| -gesture` on the verbal-system duplicates; version notes on 4 docs. Build clean (19 examples), zero glossed blocks left. Commits 5582dd4 (G0b) + B1b pending. Touched: site/scripts/{examples-render,sync-previews,build-content,parse}.mjs, pdf/build*.ps1, docs/reference/{verbal-system,ideophones,language_reference,examples}.md, docs/dictionary/dictionary.md.
