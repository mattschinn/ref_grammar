...
- [ ] **G0b. Preview auto-regen in build (single renderer) + PDF parity.** Per the 2026-06-28 decision (*auto-regenerate in build*): one Node script reuses `renderExample` to write a `<!-- preview … -->` … `<!-- /preview -->` block after each token, **into the canonical docs**, refreshed every build (build mutates tracked source — accepted). Both builds then strip the token + preview markers and **keep the rendered content** — so the Node renderer is the *only* renderer and the PowerShell side needs no layer logic (just marker-stripping). Handle inline (dictionary) vs block (grammar) token contexts. Unidirectional: previews never feed back into `examples.md`.
