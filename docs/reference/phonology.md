...
- ~~**Candidate minimal pair (*noê* / *nóè*).**~~ **Withdrawn, August 2026 — the pair was not one.** *nóè* was assigned first-syllable pitch on the strength of a **NO** way! reading of its GA source; the ordinary realization is *no WAY!*, which leaves *no* unstressed. The withdrawal rests on that GA premise alone. *(The acoustic corroboration originally cited here — a flat mean contour, +1.8 Hz early-to-late — was measured on a corrupted extraction and is withdrawn; see v4.10. Re-measurement gives a −0.7 pp centroid difference and near-identical contours across the two, which is consistent with homophony but rests on three usable repetitions and establishes nothing.)* The two headwords have been merged into one entry with two etymologies (`dictionary.md` §*noê*; author ruling 2026-08-12). The contrastiveness question above is therefore **still open and still without a candidate pair** — this one is removed from the ledger rather than answered. Search strategy unchanged.
- **Variable pitch on the new final stress.** **[Open — data gathering]** Where stress relocated to the final vowel, it consistently carries length and (relative) loudness, but pitch appears to vary: the final is sometimes low, retaining the pitch of the originally unstressed GA syllable, and sometimes high. Author's hypothesis (2026-08-12) is that the variation is genuine, carries no meaning difference, and may be conditioned by emphasis or focus. **There is currently no acoustic support for it, in either direction.** The pilot-01 figures cited here until 2026-08-17 (10 of 10 falling against 8 of 20 rising; Fisher p = 0.03; Mann–Whitney p = 0.02) were measured on a corrupted extraction that captured the recording's spoken English digit slates instead of the words, and are withdrawn. Re-measured on corrected, content-verified audio the contrast dissolves: first-syllable-pitch words rise in **2 of 9** repetitions and final-stressed words in **4 of 17** — statistically indistinguishable (Fisher p = 1.000; Mann–Whitney p = 0.196). Final-stressed words do scatter more widely (sd 3.5 st against 1.8 st, range −3.8 to +8.1), which is the one figure pointing the hypothesis's way, but Levene's test gives p = 0.484 and it is not established either. The pilot also contains no focus manipulation, so the conditioning environment remains untested regardless. If confirmed, this is a statement about optional post-lexical accent, not about lexical contrast, and bears on §3.4 and on `orthography.md` §3.7's circumflex. Protocol: `planning/audio-pipeline-plan.md` §7.
...
### v4.10, August 2026 — §5.4's acoustic figures withdrawn; the segmentation that produced them was wrong

Every number v4.9 imported from pilot 01 was measured on the wrong audio. The pipeline's segmentation stage assigned each utterance one position early, exporting the recording's spoken English digit slates in place of the conlang words; all 120 tokens in that analysis are numerals. Fixed at source (`tools/audio-split.py`), with a content gate added (`tools/audio-verify.py`) that rejects any token transcribing as its own item number. See `planning/audio-pipeline-plan.md` §7.

The error was invisible to every check that existed — utterance counts, anchor spacing, parameter-grid stability and manifest coverage are all preserved by a uniform shift — and surfaced only when the author played the audio. **The methodological lesson belongs in this document because it will recur: structural validation of a segmentation cannot establish that the right sound was measured. Only a content check can, and it has to be automatic, because neither a human across 122 files nor an LLM (which cannot hear) will catch it by listening.**

**Variable pitch on relocated final stress — the supporting statistics are withdrawn and not replaced.** Re-measured on corrected, content-verified audio, first-syllable-pitch words rise in 2 of 9 repetitions and final-stressed words in 4 of 17: Fisher p = 1.000, Mann–Whitney p = 0.196, against the 0.03 and 0.02 reported in v4.9. The wider scatter of final-stressed words survives as a tendency (sd 3.5 st against 1.8 st) but Levene's test gives p = 0.484. The item stays **[Open — data gathering]**; it is now an author's hypothesis with no acoustic evidence for or against, which is a weaker position than v4.9 described but not a refutation. Session 02's focus manipulation remains the test.

**The withdrawn *noê* / *nóè* pair stays withdrawn.** That decision rested on the GA premise (*no WAY!*, not **NO** way!), which is decidable without any recording; v4.9 said so at the time. The acoustic corroboration it cited (flat contour, +1.8 Hz) is withdrawn. Re-measurement gives a −0.7 pp centroid difference and near-identical mean contours — consistent with homophony, and at three usable repetitions not evidence of it. **§5.4 still has no candidate minimal pair, and the pitch-contrastiveness question is unchanged.**

The residue analysis in §3.4 is untouched. Nothing in it was ever derived from these measurements — §3.4's phonetic predictions remain, as v4.2 put it, falsifiable and unfalsified.

### v4.9, August 2026 — §5.4: candidate minimal pair withdrawn, variable final pitch opened

*(Superseded in part by v4.10: every acoustic figure below is withdrawn. The withdrawal of the candidate pair stands; it did not depend on them.)*

First canon change driven by recorded production (`planning/audio-pipeline-plan.md`).

**Withdrawn:** the *noê* / *nóè* candidate pitch-accent minimal pair, logged in v4.2. It was never a pair — *nóè*'s first-syllable pitch was derived from a **NO** way! reading of its GA source rather than the ordinary *no WAY!*, and across five recorded repetitions the word showed no first-syllable peak (flat, +1.8 Hz early-to-late, against 9–16 Hz declines on words that do carry an early accent). The headwords are merged in `dictionary.md` (§*noê*); `orthography.md` v3.11 replaces its §3.7 worked example accordingly. **The pitch-contrastiveness question is unchanged and still has no candidate pair** — this entry is struck from the ledger, not answered.

**Opened:** variable pitch on relocated final stress **[Open — data gathering]**. Final stress reliably carries length and relative loudness, but the pitch of the final vowel varies between low (retaining the originally unstressed GA syllable's pitch) and high. Pilot 01: first-syllable-pitch words fell in 10/10 repetitions; final-stressed words rose in 8/20 and scattered across 9 semitones (Fisher p = 0.03). *(These figures are withdrawn — see v4.10.)* Consistent with the author's hypothesis that the variation is genuine, meaning-neutral, and possibly focus-conditioned — but n = 20 cannot separate two modes from one wide distribution, and the pilot has no focus manipulation. If it holds it describes optional post-lexical accent, not lexical contrast.

**Standing caveat unchanged.** The corpus is one GA speaker who designed the language, so production cannot confirm the §3.4 residue analysis. What it did here was show a documented contrast **absent** from production and send the analysis back to a GA premise decidable without the corpus — which is the direction in which this evidence counts.
