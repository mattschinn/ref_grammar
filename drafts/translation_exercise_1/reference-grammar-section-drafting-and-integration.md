...
## 1.5 One home per fact: abstracts and stubs

Every fact has exactly one canonical location. The document family enforces this structurally, not by discipline:

**Abstracts.** Every sibling deep-dive document opens with an unnumbered **Abstract** — placed after the version header and status lines, before §1. It is 3–6 sentences of **architecture-level claims only**: what the system is, its major parts (structural counts like "four readings" are fine), and what the document holds. Prohibited in an Abstract: surface forms, IPA, inventories, paradigm cells, status tags, specific headwords — anything that can go stale at the detail level. An abstract that contains no details cannot drift.

**Stubs.** A `language_reference.md` section whose topic has a sibling document contains exactly three things and nothing else:

1. A machine-readable assembly directive on its own line: `<!-- assemble: <sibling>.md -->`
2. A blockquote opening: **Orientation abstract** — canonical content in `<sibling>.md`. This text is copied from that document's Abstract; edit it there, never here.
3. The copied Abstract text inside the same blockquote.

The stub's abstract is a **generated copy**: when a sibling's Abstract changes, the stub is regenerated by re-copying. Hand-editing a stub is a defect. One assembly directive per sibling document, placed at that sibling's primary `lang_ref` section; any other `lang_ref` section covered by the same sibling gets a **plain pointer stub** (one architecture-level sentence plus a cross-reference, no directive), so assembly never includes a document twice.

**Canonical content in `language_reference.md`.** `lang_ref` holds full-telescope canonical content only for topics **without** a sibling document (currently: nominal morphology, syntax, the lexicon framework, the visibility particles). The moment a topic spins out (§A.1, §B.1), its `lang_ref` section empties to a stub.

**Assembly model.** Two build modes, both mechanical: *standalone* `lang_ref` renders with the orientation abstracts in place (cheap LLM-context and skim layer); *full grammar PDF* renders as Part-per-document concatenation — `lang_ref` front matter and canonical sections first, then each sibling as its own Part with headings demoted one level, per-document Versioning Notes and status-tag boilerplate stripped at build time. Cross-references stay **filename-based** (`verbal-system.md §4`) in all modes; they never break under assembly because sibling-internal numbering is preserved as Part-internal numbering.

**Repetition within a document is a defect.** A system's motivation, asymmetry argument, or key example appears once in full; later sections point back rather than restate. Abstracts and pointer stubs never introduce facts absent from the canonical home.
...
The typical update merges two inputs: **draft material already present** in `language_reference.md` (or a sibling) and **user-supplied material**, usually larger and more fleshed out. Workflow A's job is to bring them together into one canon-conformant draft; nothing from the existing draft material is silently dropped — overlaps are merged, conflicts surfaced to the user.

Confirm before drafting:

- **Topic and boundaries** — what's in, what's explicitly out.
- **Destination and the spin-out rule** — which document the merged material lands in. If the topic already has a sibling document, the sibling is the canonical home. If it lives in `language_reference.md`, apply the **spin-out test**: merged material that has its own internal section structure, paradigm tables, more than a handful of open questions, or runs past roughly a page of prose **spins out** — draft it as a new sibling document (full telescope, opening Abstract per §1.5), with the `lang_ref` section becoming a stub. Material below that threshold integrates in place, with `lang_ref` remaining its canonical home. Borderline cases go to the user; the bias is toward spinning out, since the stub model makes spin-outs cheap and reversal expensive.
...
- Telescope order (§1.4). Overview paragraph first — if the overview can't be written plainly, the section isn't understood yet; surface that. A draft that will become a new sibling document also drafts its opening **Abstract** (§1.5) — architecture-level only — and the Abstract is reviewed as part of the draft.
...
- **Abstract and stub impact** — if the canonical home is a sibling document, check whether the integration changes anything the sibling's **Abstract** claims. If yes, revise the Abstract (architecture-level only, §1.5) and regenerate the `lang_ref` stub by re-copying. If no, the stub is untouched.
- **Spin-out execution** — if A.1's spin-out test fired (or the snippet lands in `language_reference.md` but exceeds the threshold), the integration creates the sibling: new document with telescope structure and Abstract; the `lang_ref` section is emptied to a stub (directive + copied Abstract); `language_reference.md` §8's document list and the "How to read this document set" front matter are updated; both documents get versioning entries recording the spin-out.
...
- Produce a **propagation checklist** for the user: stubs whose source Abstracts changed (regenerate by re-copying), open questions opened or closed elsewhere, dictionary entries implied but not made (`[Lexicon: …]` flags), and any `[New rule needed: …]` items parked for a dedicated phonology session.
...
- The **assembly script** (stub regeneration from sibling Abstracts; standalone and PDF build modes per §1.5) is not yet written. Until it exists, stub regeneration is a manual copy performed in Workflow B, and the PDF build is deferred.
...
### v1.1, June 2026 — stub-abstract model adopted

§1.5 rewritten from the summary-plus-pointer depth rule to the **stub-abstract model** (user decision, June 2026): sibling documents open with an architecture-level Abstract; `language_reference.md` sections with a sibling become generated stubs (assembly directive + copied Abstract); `lang_ref` holds canonical content only for sibling-less topics; two-mode assembly defined (standalone with abstracts; PDF as Part-per-document concatenation with filename-based cross-references). §A.1 now describes the typical merge of existing draft material with larger user input, and carries the spin-out test. §A.3, §B.1, and §B.7 updated for Abstract drafting, stub regeneration, and spin-out execution. The completed "How to read" item replaced in Open Improvements by the pending assembly script.

### v1, June 2026 — initial skill

House-style canon (Part 1), Workflow A (draft), Workflow B (integrate), as committed in the June 2026 conformance session.
